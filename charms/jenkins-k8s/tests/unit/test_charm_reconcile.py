...
def test_reconcile_admin_generates_password_when_container_credentials_missing(
    ...
    generated_password = token_hex(8)
    with (
        patch.object(
            jenkins,
        ...
        patch.object(charm.secrets, "token_hex", return_value=generated_password),
...
def test_reconcile_admin_updates_existing_secret(
    ...
    assert result == generated_password
    mock_secret.set_content.assert_called_once_with({"password": generated_password})
