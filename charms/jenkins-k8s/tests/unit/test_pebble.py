...
def _pebble_env(
    prefix: str = "", system_properties: list[str] | None = None
) -> tuple[dict[str, str], Any]:
    env = {
        "JENKINS_HOME": str(jenkins.JENKINS_HOME_PATH),
        "JENKINS_PREFIX": prefix,
        "CASC_JENKINS_CONFIG": str(jenkins.JCASC_CONFIG_PATH),
        "JENKINS_ADMIN_PASSWORD": token_hex(16),
        "CONFIGURATION_HASH": "hash123",
    }
    fake_state = SimpleNamespace(system_properties=system_properties or [])
    return env, fake_state
...
def test_get_pebble_layer_command(
    ...
    """
    env, fake_state = _pebble_env(prefix, system_properties)
...
def test_get_pebble_layer_sets_check_url_with_prefix(prefix: str, expected_url: str):
    ...
    """
    env, fake_state = _pebble_env(prefix)
