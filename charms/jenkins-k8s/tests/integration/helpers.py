...
class AuthMethod(Enum):
    ...
    The enum member *names* are descriptive labels only; the values are not
    secrets and are only used for log correlation.
    """

    TOKEN = "auth-method-1"  # nosec B105: enum label, not a credential.
    PASSWORD = "auth-method-2"  # nosec B105: enum label, not a credential.
