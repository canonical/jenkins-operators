...
The [jenkins-k8s-operator](https://github.com/canonical/jenkins-operators) charm uses [Juju constraints](https://canonical.com/juju/docs/juju-cli/3.6/reference/constraint/) to limit the amount of memory a charm can use. To deploy the charm with constraints, use the `--constraints "<key>=<value>"` option when running `juju deploy`:
```bash
